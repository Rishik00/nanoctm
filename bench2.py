...
def run_bench(
    ...
    *,
    ...
    print(
        f"  {name:<52}  {sps:8.1f} sps   {mps:6.2f} ms/step   {tokens_sec:.2f}M tok/s"
    )
    return sps
...
class DeepOptCTM(NanoCTM):
    ...
    def __init__(self, config: CTMConfig):
        ...
        if cfg.neuron_select_type != "random-pairing":
            il_a = self.idx_left_action[i_a]
            ir_a = self.idx_right_action[j_a]
            il_o = self.idx_left_out[i_o]
            ir_o = self.idx_right_out[j_o]
        else:
            il_a = self.idx_left_action
            ir_a = self.idx_right_action
            il_o = self.idx_left_out
            ir_o = self.idx_right_out
    ...
    # ------------------------------------------------------------------
    def _attn(
        self, q_vec: torch.Tensor, k: torch.Tensor, v: torch.Tensor, B: int
    ) -> torch.Tensor:
        """
    ...
    def forward(self, x: torch.Tensor):
        ...
        kv = self.kv_proj(kv)
        kv = (kv.transpose(1, 2) + self.pos_embedding(kv.transpose(1, 2))).transpose(
            1, 2
        )
        ...
        da_a = torch.zeros(
            B,
            cfg.synch_rep_size_action,
            device=x.device,
            dtype=x.dtype if not x.is_floating_point() else torch.float32,
        )
        ...
        _, da_o, db_o = self._sync(
            activated_state, da_o, db_o, r_out, self.sync_il_o, self.sync_ir_o
        )
        ...
        for t in range(cfg.iterations):
            sync_a, da_a, db_a = self._sync(
                activated_state, da_a, db_a, r_action, self.sync_il_a, self.sync_ir_a
            )
...
def main():
    ...
    print(
        f"synch_rep  first-last={cfg.synch_rep_size_action}  "
    ...
    rec(
        "NanoCTM+fl+B256  [eager baseline]",
        run_bench(
            lambda: NanoCTM(cfg),
            256,
            "NanoCTM+first-last+B=256  [eager baseline]",
            use_compile=False,
            use_bf16=False,
            fused_optim=False,
        ),
    ...
    rec(
        "NanoCTM+fl+B256+compile+bf16+fused",
        run_bench(
            lambda: NanoCTM(cfg),
            256,
            "+fused AdamW",
    ...
    print("\n── scale batch + deep optimisations ───────────────────────────────")
    rec(
        "NanoCTM+fl+B2048+compile+bf16+fused",
        run_bench(
            lambda: NanoCTM(cfg),
            2048,
            "NanoCTM+first-last+B=2048",
            use_compile=True,
            use_bf16=True,
            fused_optim=True,
        ),
    ...
    rec(
        "DeepOpt+fl+B2048+compile+bf16+fused",
        run_bench(
            lambda: DeepOptCTM(cfg),
            2048,
            "DeepOpt+first-last+B=2048  (+KV hoist)",
            use_compile=True,
            use_bf16=True,
            fused_optim=True,
        ),
    ...
    rec(
        "NanoCTM+rp+B2048+compile+bf16+fused",
        run_bench(
            lambda: NanoCTM(cfg_rp),
            2048,
            "NanoCTM+random-pair+B=2048",
            use_compile=True,
            use_bf16=True,
            fused_optim=True,
        ),
    ...
    rec(
        "DeepOpt+rp+B2048+compile+bf16+fused",
        run_bench(
            lambda: DeepOptCTM(cfg_rp),
            2048,
